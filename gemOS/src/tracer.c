...
//printk("count(40, 40) : %d\n", count);
	...
	//printk("Write pos before: %x\n", i);

    if((filep -> mode != O_RDWR) && (filep -> mode != O_WRITE)) return -EINVAL;
    if(count <= 0) return 0;
    if(free_space <= 0) return 0;

    //While tracing is enabled, we use trace_buffer_write() to write to the trace buffer.
    //However, the buffer we pass to this function during tracing is a kernel space buffer.
    //Thus, is_valid_mem_range() won't be able to find it and it will return -EBADMEM.
    //
    //So, disable memory checking while tracing is in progress
    if((ctx->st_md_base == NULL) && (ctx->ft_md_base == NULL))
    {
    ...
    filep -> trace_buffer -> remaining_data += count;
	//printk("Write pos after: %x\n", filep -> trace_buffer -> write_pos);

	//printk("buff[0]: %x, buff[8]: %x, buff[16]: %x, buff[24]: %x, buff[32]: %x\n", ((u64*)buff)[0], ((u64*)buff)[1], ((u64*)buff)[2], ((u64*)buff)[3], ((u64*)buff)[4]);
...
int perform_tracing(u64 syscall, u64 param1, u64 param2, u64 param3, u64 param4){
	...
	total_size += param_size;
        ...
        if(current->st_md_base->tracing_mode == FILTERED_TRACING){
                ...
                //printk("Full tracing mode, syscall: %d\n", syscall);
                strace_buff[0] = (total_size-1)*8;
                strace_buff[1] = syscall;
...
{
	//printk("Inside remove_strace. syscall num: %x\n", syscall_num);
	struct strace_info *info = find_strace_info(current, syscall_num);
	if(!info)
	{
		//printk("info struct corresponding syscall num: %x not found\n", syscall_num);
            return -1;
	}
...
int sys_read_strace(struct file *filep, char *buff, u64 count)
{
	...
	//printk("size(40, 40) : %d\n", size);
            read_pos += 8;
	    read_pos = read_pos%4096;
     
	//printk("source_buffer[read_pos/8] : %x %x %x %x\n", ((u64 *)source_buffer)[(read_pos%4096)/8], (((u64 *)source_buffer)[((read_pos+8)%4096)/8]), (((u64 *)source_buffer)[((read_pos+16)%4096)/8]), (((u64 *)source_buffer)[((read_pos+24)%4096)/8]));
